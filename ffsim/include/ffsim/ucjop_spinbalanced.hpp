    ...
    bool ordered)
{
    ...
    for (const auto& p : *interaction_pairs)
    {
        ...
        {
            throw std::runtime_error("When specifying alpha-alpha or beta-beta interaction pairs, "
...
struct UCJOpSpinBalanced
{
     ...
     */
    UCJOpSpinBalanced(const Tensor<Complex, 4>& diag_coulomb_mats,
     ...
     */
    static size_t
    n_params(uint64_t norb, size_t n_reps,
    ...
    from_parameters(const VectorXcd& params, uint64_t norb, size_t n_reps,
                    const std::array<std::optional<std::vector<std::pair<uint64_t, uint64_t>>>, 2>&
                        interaction_pairs,
                    bool with_final_orbital_rotation)
    {
        ...
        {
            throw std::runtime_error("Expected " + std::to_string(expected_params) +
        ...
        Tensor<Complex, 4> diag_coulomb_mats(static_cast<long>(n_reps), static_cast<long>(2),
                                             static_cast<long>(norb), static_cast<long>(norb));
        diag_coulomb_mats.setZero();
        Tensor<Complex, 3> orbital_rotations(static_cast<long>(n_reps), static_cast<long>(norb),
     ...
     */
    VectorXcd
    to_parameters(const std::array<std::optional<std::vector<std::pair<uint64_t, uint64_t>>>, 2>&
                      interaction_pairs) const
    {
        size_t n_reps = diag_coulomb_mats.dimension(0);
        uint64_t norb = diag_coulomb_mats.dimension(3);
        auto total = n_params(norb, n_reps, interaction_pairs, final_orbital_rotation.has_value());
     ...
     */
    static UCJOpSpinBalanced
    from_t_amplitudes(const Tensor<Complex, 4>& t2, const std::optional<MatrixXcd>& t1,
                      std::optional<size_t> n_reps,
                      const std::array<std::optional<std::vector<std::pair<uint64_t, uint64_t>>>,
                                       2>& interaction_pairs,
                      double tol)
    {
        validate_interaction_pairs(interaction_pairs[0], false);
        ...
        if (n_reps.has_value())
        {
            ...
            Eigen::DSizes<Index, 3> slice_shape{static_cast<Index>(n_terms),
                                                static_cast<long>(norb), static_cast<long>(norb)};
            auto diag_slice = diag_coulomb_mats_reshaped.slice(
                array<Index, 3>{0, 0, 0},
                array<Index, 3>{static_cast<long>(n_terms), static_cast<long>(norb),
                                static_cast<long>(norb)});
                ...
                const std::optional<std::vector<std::pair<uint64_t, uint64_t>>>& pairs)
        {
            ...
            std::vector<std::vector<bool>> mask(norb, std::vector<bool>(norb, false));
        ...
        return UCJOpSpinBalanced(diag_coulomb_mats_stacked, orbital_rotations_reshaped,
     ...
     */
    size_t norb() const { return diag_coulomb_mats.dimension(3); }

    /**
     * @brief Returns the number of repetitions (n_reps).
     */
    size_t n_reps() const { return diag_coulomb_mats.dimension(0); }
};
