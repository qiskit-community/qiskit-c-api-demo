...
class DiagCoulombEvolutionJW
{
  public:
    ...
    DiagCoulombEvolutionJW(
        uint64_t norb, const Mat &mat, double time, bool z_representation
    )
    ...
    diag_coulomb_evolution_num_rep_jw(const std::vector<uint32_t> &qubits) const
    {
        ...
        auto [mat_aa, mat_ab, mat_bb] = [&]() {
            if (mat.type == MatType::Single) {
                ...
                return std::make_tuple(
                    mat.triple[0].value_or(
                        MatrixXcd::Zero(
                            static_cast<Eigen::Index>(norb),
                            static_cast<Eigen::Index>(norb)
                        )
                    ),
                    mat.triple[1].value_or(
                        MatrixXcd::Zero(
                            static_cast<Eigen::Index>(norb),
                            static_cast<Eigen::Index>(norb)
                        )
                    ),
                    mat.triple[2].value_or(
                        MatrixXcd::Zero(
                            static_cast<Eigen::Index>(norb),
                            static_cast<Eigen::Index>(norb)
                        )
        ...
        for (int sigma = 0; sigma < 2; ++sigma) {
            ...
            for (size_t i = 0; i < norb; ++i) {
                if (std::abs(
                        this_mat(static_cast<Index>(i), static_cast<Index>(i)).real()
                    ) > 1e-12 ||
                    std::abs(
                        this_mat(static_cast<Index>(i), static_cast<Index>(i)).imag()
                    ) > 1e-12) {
                    instructions.push_back(
                        {"rz",
                         ...
                         std::vector<double>{
                             -0.5 *
                             this_mat(static_cast<Index>(i), static_cast<Index>(i))
                                 .real() *
                             time
                         }}
                    );
                ...
                for (size_t j = i + 1; j < norb; ++j) {
                    if (std::abs(this_mat(static_cast<Index>(i), static_cast<Index>(j))
                                     .real()) > 1e-12 ||
                        std::abs(this_mat(static_cast<Index>(i), static_cast<Index>(j))
                                     .imag()) > 1e-12) {
                        instructions.push_back(
                            {"cp",
                             {static_cast<unsigned int>(qubits[i + sigma * norb]),
                             ...
                             std::vector<double>{
                                 -1.0 *
                                 this_mat(static_cast<Index>(i), static_cast<Index>(j))
                                     .real() *
                                 time
                             }}
                        );
        ...
        for (size_t i = 0; i < norb; ++i) {
            if (std::abs(mat_ab(static_cast<Index>(i), static_cast<Index>(i)).real()) >
                    1e-12 ||
                std::abs(mat_ab(static_cast<Index>(i), static_cast<Index>(i)).imag()) >
                    1e-12) {
                instructions.push_back(
                    {"cp",
                     ...
                     std::vector<double>{
                         -1.0 *
                         mat_ab(static_cast<Index>(i), static_cast<Index>(i)).real() *
                         time
                     }}
                );
            }
            for (size_t j = i + 1; j < norb; ++j) {
                if (std::abs(
                        mat_ab(static_cast<Index>(i), static_cast<Index>(j)).real()
                    ) > 1e-12 ||
                    std::abs(
                        mat_ab(static_cast<Index>(i), static_cast<Index>(j)).imag()
                    ) > 1e-12) {
                    instructions.push_back(
                        {"cp",
                         {static_cast<unsigned int>(qubits[i]),
                         ...
                         std::vector<double>{
                             -1.0 *
                             mat_ab(static_cast<Index>(i), static_cast<Index>(j))
                                 .real() *
                             time
                         }}
                    );
                ...
                if (std::abs(
                        mat_ab(static_cast<Index>(j), static_cast<Index>(i)).real()
                    ) > 1e-12 ||
                    std::abs(
                        mat_ab(static_cast<Index>(j), static_cast<Index>(i)).imag()
                    ) > 1e-12) {
                    instructions.push_back(
                        {"cp",
                         ...
                         std::vector<double>{
                             -1.0 *
                             mat_ab(static_cast<Index>(j), static_cast<Index>(i))
                                 .real() *
                             time
                         }}
                    );
            ...
            for (size_t j = i + 1; j < 2 * norb; ++j) {
                const MatrixXcd this_mat = (i < norb && j < norb)     ? mat_aa
                                           : (i >= norb && j >= norb) ? mat_bb
                                                                      : mat_ab;
                const auto val = this_mat(
                    static_cast<Index>(i % norb), static_cast<Index>(j % norb)
                );
