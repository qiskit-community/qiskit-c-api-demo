...
VectorXd orbital_rotation_to_parameters(const MatrixXcd &orbital_rotation, bool real)
{
            ...
            for (int j = 0; j < orbital_rotation.cols(); ++j) {
                if (std::abs(orbital_rotation(i, j).imag()) > 1e-12) {
                    throw std::runtime_error(
                        "real was set to True, but the orbital "
    ...
    for (size_t idx = 0; idx < triu_indices_no_diag.size(); ++idx) {
        ...
        params(static_cast<Index>(idx)) =
            mat(static_cast<Index>(i), static_cast<Index>(j)).real();
    }
    ...
    if (!real) {
        ...
        for (size_t idx = 0; idx < triu_indices.size(); ++idx) {
            ...
            params(static_cast<Index>(idx + triu_indices_no_diag.size())) =
                mat(static_cast<Index>(i), static_cast<Index>(j)).imag();
        }
    }
...
MatrixXcd orbital_rotation_from_parameters(const VectorXcd &params, int norb, bool real)
{
    ...
    if (!real) {
        ...
        for (size_t idx = 0; idx < triu_indices.size(); ++idx) {
            auto [i, j] = triu_indices[idx];
            Complex imag_param_val =
                params[static_cast<Index>(idx + triu_indices_no_diag.size())];
            generator(static_cast<Index>(i), static_cast<Index>(j)) =
                Complex(0.0, imag_param_val.real());
            generator(static_cast<Index>(j), static_cast<Index>(i)) =
                Complex(0.0, imag_param_val.real());
        }
    }
