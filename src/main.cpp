...
// b) Real: build LUCJ circuit -> transpile -> run on backend with Sampler -> get counts
    ...
    for (uint64_t i_recovery = 0; i_recovery < n_recovery; ++i_recovery)
    {
        ...
        if (sqd_data.mpi_rank == 0)
        {
            ...
            // Subsample to a single batch of fixed size for SBD, to cap IO/compute per
            // iteration.
            Qiskit::addon::sqd::subsample(batch, postselected_bitstrings, postselected_probs,
                                          samples_per_batch, rng);
            // Write alpha-determinants file for SBD input (includes run id / iteration
            // for traceability).
            diag_data.adetfile =
                write_alphadets_file(sqd_data, norb, num_elec_a, bitsets_to_bitstrings(batch),
