...
struct UArrayString
{
    ...
    bool operator==(const UArrayString& other) const { return to_string() == other.to_string(); }
    ...
    std::vector<size_t> get_indices(const size_t from, const size_t unti, const bool value) const;
...
bitstring_matrix_to_ci_strs(const std::vector<BitString>& bitstring_matrix, bool open_shell = false)
{
    ...
    for (size_t config = 0; config < num_configs; ++config)
    {
        ...
        for (size_t i = 0; i < norb; ++i)
            if (row[i + norb])
                ci_str_right[config] += static_cast<uint64_t>(std::pow(2, norb - 1 - i));
    }
    ...
    std::vector<uint64_t> result_left(unique_ci_str_left.begin(), unique_ci_str_left.end());
    std::vector<uint64_t> result_right(unique_ci_str_right.begin(), unique_ci_str_right.end());
...
std::vector<uint64_t> //
get_unique_ci_strs_with_HF(const SQD& sqd_data, const std::vector<uint64_t>& left_ci_strs,
...
void write_bytestrings_to_file(const std::vector<std::vector<uint8_t>>& byte_strings,
                                 ...
                                 const size_t maximum_numbers_of_ctrs, const size_t i_recovery)
{
    ...
    log(sqd_data, {"number of items in left ci_strs:", std::to_string(ci_strs.first.size())});
    log(sqd_data, {"number of items in right ci_strs:", std::to_string(ci_strs.second.size())});
